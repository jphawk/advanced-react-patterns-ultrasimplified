import React, { useCallback, useLayoutEffect, useState, useRef, useEffect, useReducer } from "react"
...
// custom hook for prevCount value
...
const internalReducer = ({count, countTotal}, {type, value}) => {
  switch (type) {
    case 'clap':
      return {
...
const useClapState = ( {initialState = INITIAL_STATE} = {}, reducer = internalReducer) => {
  ...
  const [clapState, dispatch] = useReducer(reducer, userInitialStateRef.current)
  ...
  const reset = useCallback(() => {
    if (prevCount !== count) {
      dispatch({type: 'reset', value: userInitialStateRef.current})
      ++resetRef.current
    }
...
const Usage = () => {
  ...
  return (
    <div>
      ...
      >
        <ClapCount count={count} setRef={setRef} data-refkey="clapCountRef" {...getCounterProps()} />
      ...
      <section>
        ...
        <pre className={userStyles.resetMsg}>
          {isClappedTooMuch ? 'You clapped too much! Stop it!' : ''}
        </pre>
      </section>
    </div>
  )
