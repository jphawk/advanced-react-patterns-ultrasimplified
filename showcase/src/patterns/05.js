...
const MediumClap = ({children, onClap, style: userStyles, className, values = null}) => {
  ...
  const handleClick = () => {
    ...
    :
      setClapState(prevState => ({
        ...
        countTotal: count < prevState.countTotal ? prevState.countTotal + 1 : prevState.countTotal,
        isClicked: true
      }))
  }
  ...
  const memorizedValue = useMemo(() => (
    {...getState(), setRef,}
    ), [getState, clapState, setRef])

  const newClassName = [styles.clap, className].join(' ').trim()
  return (
...
const ClapCount = ({style: userStyles = {}, className }) => {
  ...
  const newClassName = [styles.count, className].join(' ').trim()
  return <span style={userStyles} ref={setRef} data-refkey="clapCountRef" className={newClassName}> + {count}</span>
}

const ClapIcon = ({style: userStyles = {}, className}) => {
  const {isClicked} = useContext(MediumClapContext)
  const newClassName = [styles.icon, isClicked ? styles.checked: '', className].join(' ').trim()
  return (
    <span>
      <svg
          id='clapIcon'
...
const CountTotal = ({className}) => {
  const {countTotal, setRef} = useContext(MediumClapContext)
  const newClassName = [styles.total, className].join(' ').trim()
  return <span ref={setRef} data-refkey="clapTotalRef" className={newClassName}>{countTotal}</span>
}
...
const MAX_CLAP_VALUE = 10

const Usage = () => {
  const [state, setState] = useState(INITIAL_STATE)
  const handleClap = () => {
    setState(({count, countTotal}) => ({
      count: Math.min(count + 1, MAX_CLAP_VALUE ),
      countTotal: count < MAX_CLAP_VALUE ? countTotal + 1 : countTotal,
      isClicked: true
    }))
  }
  return (
